...
const coachId = parseInt(currentRoute.value.params.id as string)
...
const isCoachFetched = ref(false)
const dbError = ref()
...
const getCoachDetails = () => {
    axios.get(`https://akademia.website/api/coach/${coachId}`).then((result) => {
        ...
        result.data.coach.branches.forEach((branch : any) => {
            coachDetails.value.branchIds.push(branch.id)
        });
        console.log(coachDetails.value);
...
<template>
    ...
    <div class="w-12 md:w-10 m-auto p-2 md:p-5 branches">
        ...
        <div v-if="isBranchesFetched && isCoachFetched">
            <FormKit type="form" :actions="false" v-model="coachDetails" @submit="updateCoach">
                ...
                <div class="mt-3 col-12">
                    ...
                    <div class="mt-3 col-12 md:col-6">
                        ...
                        <FormKit prefix-icon="telephone" id="phone" type="text" label="اسم المدرب" placeholder="أدخل رقم هاتف المدرب" name="phone" validation="required|length:10" />
                    </div>
