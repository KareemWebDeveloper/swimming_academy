...
const createSubscription = (req : any) => {
    ...
    if(isCustomerRegistered.value && createCustomerSubscription.value){
    ...
    axios.post('https://akademia.website/api/createSubscription' , req).then((result) => {
        ...
        if(customerDetails && customerDetails.customer_name){
            ...
            createdSubscription.value.customer_name = subscriptionFields.value.customer_name
        }
...
<template>
    <Dialog v-model:visible="isDialogVisible" @after-hide="handleDialogClosed" modal header="بيانات الاشتراك" :style="{ width: '50vw' }" :breakpoints="{ '960px': '75vw', '641px': '100vw' }">
        <div class="flex align-items-center justify-content-center">
            ...
            <div class="flex align-items-center mx-5 justify-content-center flex-column">
                ...
                <p v-if="createdSubscription.customer_name">{{ createdSubscription.customer_name }}</p>
            </div>
            <div class="flex align-items-center mx-5 justify-content-center flex-column">
                <div class="flex align-items-center my-2">
                    <span class="material-symbols-outlined mx-1 text-2xl">
    ...
    <div class="w-12 md:w-10 m-auto p-2 md:p-5 customers">
        <h1 class="p-4 text-center textColor">تسجيل تمرين فردي</h1>
        ...
        <!-- Create customer private subscription -->

        <div v-else-if="isCustomerRegistered  && isBranchesFetched && isCustomersFetched" class="subscription w-full m-auto p-2 md:p-4 customers">
            <FormKit type="form" v-model="subscriptionFields" :actions="false" @submit="createSubscription">
                <div class="flex grid w-full">
                    ...
                    <div class="col-12">
                        ...
                        <FormKit
                        type="autocomplete" validation="required" name="branch_id"  placeholder="اختر الفرع الذي سيحضر فيه المشترك" label="الفرع"
                        :options="branches"
                        />
                    </div>
                    ...
                    <div class="mt-3 col-12 md:col-6">
                        ...
                        <div class="mt-3 col-12 md:col-6">
                            <div class="flex align-items-center">
                                <label for="academy" class="px-3 py-1 text-white text-sm">اسم الأكاديمية</label>
                            </div>
                            <FormKit prefix-icon="text" id="academy" type="text" label="اسم الأكاديمية" placeholder="أدخل اسم الأكاديمية" name="academy_name" validation="required|length:2" />
                        </div>
                        <div class="mt-3 col-12 md:col-6">
                            <div class="flex align-items-center">
