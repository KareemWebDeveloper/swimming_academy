...
<template>
    ...
    <div>
        ...
        <template #header>
            <div class="flex flex-column lg:flex-row justify-content-between align-items-center">
                <div class="flex align-items-center">
                    <Button type="button" v-if="isEmpAuthorizedFor(empPermissions , 'تسجيل و تعديل التمرينات الفردية' , UserType)" class="mb-3 lg:mb-0 mx-2" @click="push('/privateSubscriptions/create')" label="تسجيل مشترك" />
                    <Button type="button" v-if="isEmpAuthorizedFor(empPermissions , 'تسجيل و تعديل التمرينات الفردية' , UserType)" :disabled="selectedCustomers.length == 0" severity="danger" class="mb-3 lg:mb-0 mx-2" label="حذف المحدد" />
                </div>
